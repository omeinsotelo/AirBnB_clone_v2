...
def do_deploy(archive_path):
    ...
    try:
        ...
        command = "sudo tar -xzf /tmp/{} -C ".format(fullName)
        command += "/data/web_static/releases/{}/".format(fileName)
        run(command)

        # Delete file from server
        command = "sudo rm /tmp/{}".format(fullName)
        run(command)

        # Move all the files into the dir releases/web_static<number>
        command = "sudo mv /data/web_static/releases/{}".format(fileName)
        command += "/web_static/* "
        command += "/data/web_static/releases/{}/".format(fileName)
        run(command)

        # Delete the folder that create the uncompres process
        command = "sudo rm -rf /data/web_static/releases/{}".format(fileName)
        command += "/web_static"
        run(command)
        ...
        command = "sudo ln -s /data/web_static/releases/{}/ ".format(fileName)
...
def deploy():
    ...
    using the function do_deploy:
    """
    # Compress all the information get the name .tgz
